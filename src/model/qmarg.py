...
def make_model(args):
    return Net(
        ...
        in_glo_channel=args.in_glo_channel,
...
class Net(nn.Module):
    def __init__(self, in_channel, in_glo_channel, out_channel, n_feats=64, n_resblocks=16, res_scale=0.1, scale=2):
    ...
    def forward(self, source):
        x = (source - 0.5) / 0.5
        resized = x[:,0:3,:,:]
        resized = torch.nn.functional.interpolate(input=resized, size=[112, 112], mode='bicubic')
