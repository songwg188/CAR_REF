...
from model import basicblock as B
import numpy as np
...
'''
def make_model(args):
    quality = eval(args.test_quality) if isinstance(args.test_quality, str) else args.test_quality
    quality = quality[0]
    return UNetRes(qt=quality,in_nc=args.in_channel, out_nc=args.n_colors, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose')


class UNetRes(nn.Module):
    def __init__(self, qt=5,in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
