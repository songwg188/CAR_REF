...
def calc_PSNR(hq, gt, rgb_range=255, shave=0):
    # import pdb;pdb.set_trace()
    hq_array = hq.squeeze(0).mul(255 / rgb_range).byte().permute(1, 2, 0).cpu().numpy()
    gt_array = gt.squeeze(0).mul(255 / rgb_range).byte().permute(1, 2, 0).cpu().numpy()

    return PSNR(hq_array, gt_array)
