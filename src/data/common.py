...
def scale_augment(img):
    ...
    scales = [1.0, 0.7, 0.5]
    s = scales[random.randint(0, len(scales)-1)]
    img = transform.resize(img, (np.int(img.shape[0] * s), np.int(img.shape[1] * s)), order=3)
...
def scale_augment_ref(img_input, img_ref):
    '''reference based multi-scale augmentation'''
    scales = [1.0, 0.7, 0.5]
    s = scales[random.randint(0, len(scales)-1)]
    img_input = transform.resize(img_input, (np.int(img_input.shape[0] * s), np.int(img_input.shape[1] * s)), order=3)
    img_input = np.uint8(img_input * 255)

    img_ref = transform.resize(img_ref, (np.int(img_ref.shape[0] * s), np.int(img_ref.shape[1] * s)), order=3)
    img_ref = np.uint8(img_ref * 255)
    return img_input, img_ref


def augment_img_ref(img_input, img_ref, mode='8'):
    '''flip and/or rotate the image randomly'''
    if mode == '2':
        mode = random.randint(0, 1)
    elif mode == '4':
    ...
    if mode == 0:
        ...
        return np.fliplr(img_input), np.fliplr(img_ref)
    elif mode == 2:
        return np.rot90(img_input, k=2), np.rot90(img_ref, k=2)
    elif mode == 3:
        return np.fliplr(np.rot90(img_input, k=2)), np.fliplr(np.rot90(img_ref, k=2))
    elif mode == 4:
        return np.rot90(img_input, k=1), np.rot90(img_ref, k=1)
    elif mode == 5:
        return np.fliplr(np.rot90(img_input, k=1)), np.fliplr(np.rot90(img_ref, k=1))
    elif mode == 6:
        return np.rot90(img_input, k=3), np.rot90(img_ref, k=3)
    elif mode == 7:
        return np.fliplr(np.rot90(img_input, k=3)), np.fliplr(np.rot90(img_ref, k=3))
 

def random_crop_ref(img_input, img_ref, size):
    '''crop image patch randomly'''
    if isinstance(size, numbers.Number):
        size = (int(size), int(size))
    h, w = img_input.shape[0:2]
    ph, pw = size
    rnd_h = random.randint(0, h - ph)
    rnd_w = random.randint(0, w - pw)
    img_patch_input = img_input[rnd_h:rnd_h + ph, rnd_w:rnd_w + pw, ...]
    img_patch_ref = img_ref[rnd_h:rnd_h + ph, rnd_w:rnd_w + pw, ...]
    return img_patch_input, img_patch_ref

def uint2tensor(img, rgb_range):
    if img.ndim == 2:
        img = img[:, :, np.newaxis]
    img = skimage.img_as_float32(img)
    img = torch.from_numpy(np.ascontiguousarray(img.transpose(2, 0, 1))).float()
    return img * rgb_range


def tensor2uint(img, rgb_range):
    img = img.data.squeeze().cpu().numpy().astype(np.float32)
    if img.ndim == 3:
        img = img.transpose(1, 2, 0)
    elif img.ndim == 4:
        img = img.transpose(0, 2, 3, 1)
    img = img.clip(0, 1) * 255 / rgb_range
    img = img.round().astype(np.uint8)
    return img
...
def get_LQ(imgGT, quality, colorMode='L'):
    ...
    assert h % 8 == 0, f'The image height should be times of 8, but {h}.'
    assert w % 8 == 0, f'The image weight should be times of 8, but {w}.'
    cnt1 = h // 8
    cnt2 = w // 8
    QMimg = np.tile(QM, (cnt1,cnt2,1))
    
    return imgIn, QMimg, quality
